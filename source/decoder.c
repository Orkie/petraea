...
static inline void arm_decode_operand2_immRot(pt_arm_operand2* dest, uint16_t op) {
...
static inline void arm_decode_operand2_immVal(pt_arm_operand2* dest, uint16_t op) {
  dest->is_immediate = true;
  dest->op.imm.value = op & 0xFFF;
}

static inline void arm_decode_operand2_reg(pt_arm_operand2* dest, uint16_t op) {
...
static inline bool instr_is_data_processing(uint32_t i) {
  return (i & (0x3 << 26)) == 0x0;
}

static inline void decode_data_processing(pt_arm_instr_data_processing* dest, uint32_t i) {
...
static inline bool instr_is_single_data_transfer(uint32_t i) {
  return ((i&0x0E000000) == 0x04000000) // immediate offset
    || (((i&0x0E000000) == 0x06000000) && ((i&0x10) == 0x0));
}

static inline void decode_single_data_transfer(pt_arm_instr_single_data_transfer* dest, uint32_t i) {
...
static inline bool instr_is_undefined(uint32_t i) {
  return ((i&0x0E000000) == 0x06000000) && ((i&0x10) == 0x10);
}

static inline bool instr_is_branch_exchange(uint32_t i) {
  return (i&0x0FFFFFF0) == 0x12FFF10;
}

static inline bool instr_is_branch(uint32_t i) {
  return (i&0x0E000000) == 0x0A000000;
}

static inline void decode_branch(pt_arm_instr_branch* dest, uint32_t i) {
...
static inline bool instr_is_halfword_data_transfer(uint32_t i) {
  return (((i>>25)&0x7) == 0x0) && (((i>>4)&0xF) == 0xD || ((i>>4)&0xF) == 0xB || ((i>>4)&0xF) == 0xF);
}

static inline void decode_halfword_data_transfer(pt_arm_instr_halfword_data_transfer* dest, uint32_t i) {
  const bool p = (i >> 24) & 0x1;
  const bool w = (i >> 21) & 0x1;
...
static inline bool instr_is_swap(uint32_t i) {
  return (i&0xFC000F0) == 0x1000090 || (i&0xFC000F0) == 0x1400090;
}

static inline void decode_swap(pt_arm_instr_swap* dest, uint32_t i) {
  dest->transfer_byte = (i >> 22) & 0x1;
  dest->value = i&0xF;
  dest->dest = (i>>12)&0xF;
  dest->addr = (i>>16)&0xF;
...
static inline bool instr_is_block_data_transfer(uint32_t i) {
  return ((i&0x0E000000) == 0x08000000);
}

static inline void decode_block_data_transfer(pt_arm_instr_block_data_transfer* dest, uint32_t i) {
...
static inline bool instr_is_coprocessor_register_transfer(uint32_t i) {
  return ((i&0x0F000010) == 0x0E000010);
}

static inline void decode_coprocessor_register_transfer(pt_arm_instr_coprocessor_register_transfer* dest, uint32_t i) {
  dest->crm = i&0xF;
...
static inline bool instr_is_multiply(uint32_t i) {
  return ((i&0x0FC000F0) == 0x90);
}

static inline void decode_multiply(pt_arm_instr_multiply* dest, uint32_t i) {
  dest->rd = (i>>16)&0xF;
  dest->rm = i&0xF;
  dest->rs = (i>>8)&0xF;
  dest->rn = (i>>12)&0xF;
...
static inline bool instr_is_multiply_long(uint32_t i) {
  return ((i&0x0F8000F0) == 0x800090);
}

static inline void decode_multiply_long(pt_arm_instr_multiply_long* dest, uint32_t i) {
  dest->rd_hi = (i>>16)&0xF;
  dest->rd_lo = (i>>12)&0xF;
  dest->rs = (i>>8)&0xF;
  dest->rm = i&0xF;
...
pt_arm_instruction_type pt_arm_identify_instruction(uint32_t i) {
  if(instr_is_single_data_transfer(i)) {
    ...
    return INSTR_HALFWORD_DATA_TRANSFER;
